...
1. `pilot secret set` prompts for the value and stores it locally, keyed by the app the compose file names.
...
| Rule | Why |
| --- | --- |
| the user runs `pilot secret set`, not you | the value must not pass through the model or the conversation |
...
| a `secret://` reference with nothing stored | the deploy has no value for it | ask the user to run `pilot secret set <name>` |
